...
brfss %>% 
  ...
  geom_point(data = brfss_excellent, aes(x = year, y = avg_data_value, group = state, color = state), show.legend = FALSE) +
  labs(
    title = "Change in Data Values by Year ", 
    subtitle = "States with >= 1 Year of Reported \"Excellent\" Overall Health",
    y = "Average State Value",
    x = "Year",
    caption = "Points indicate the average value of only sites reporting excellent overall health during the year which they occurred"
  )
## `summarise()` has grouped output by 'year'. You can override using the `.groups` argument.
```
